    ...
    public function init()
    {
        ...
        if ($this->verification->id) {
			...
			if ($programSettings->allow_educator_signoff_login) {
				$defaultValue = "Password";
			} else if ($programSettings->allow_educator_signoff_signature) {
				$defaultValue = "Signature";
			} else if ($programSettings->allow_educator_signoff_email) {
				$defaultValue = "Email";
			} else if ($programSettings->allow_educator_signoff_attachment) {
        ...
        switch ($verification->type->name) {
            case 'Password':
				...
				if (!$user->id) {
					...
					break;
				} else if ($user->getCurrentRoleData()->program->id != \Fisdap\Entity\ProgramLegacy::getCurrentProgram()->id) {
					$valid = false;
					$errors['username'] = "This username does not belong to the student's program. Please try a different username.";
					break;
				} else if (!\Fisdap\Entity\User::authenticate_password($values['username'], $values['password'])) {
					$valid = false;
