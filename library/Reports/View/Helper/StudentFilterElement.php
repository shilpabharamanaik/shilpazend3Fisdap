...
class Reports_View_Helper_StudentFilterElement extends Zend_View_Helper_FormElement
{
    public function studentFilterElement($name, $value = null, $attribs = null)
    {
        ...
        if ($defaults['year'] != 'all') {
            $yearOptions[$defaults['year']] = $defaults['year'];
        }
        $monthOptions = array(
            'all' => 'All Months',
            1 => 'Jan',
        ...
        if ($defaults['classyear'] != 'all') {
            $classSectionYearOptions[$defaults['classyear']] = $defaults['classyear'];
        }

        $classSectionOptions = array('all' => 'All'); //'All Sections');
        if ($defaults['class'] != 'all') {
            $classSectionOptions[$defaults['class']] = $defaults['class'];
        }
        
        $studentOptions = array('all' => 'All');
        if ($defaults['student'] != 'all') {
            $studentOptions[$defaults['student']] = $defaults['student'];
        }
