		...
		foreach($definitions as $def){
			...
			if($items){
				foreach($items as $item){
					...
					if($evalType == "StudentLegacy"){
					...
					else if($evalType == "InstructorLegacy"){
						$instructorEvaluator = \Fisdap\EntityUtils::getEntity("InstructorLegacy", $item->evaluator_id);
						
						if($item->passed){
							if($item->confirmed){$instructorCheckoff = "<img class='checkmark' src='/images/icons/checkmark.png'> ";}
							else {$instructorCheckoff = "<img class='incomplete' src='/images/icons/incomplete.png'> ";}
						}
						else {$instructorCheckoff = "<div class='missing-checkoff'></div> ";}
						...
						if(!$item->confirmed){$noConfirmationAlert = true;}
						else {
							if($item->passed){
		...
		foreach($definitions as $def){
			...
			foreach($items as $item){
				...
				if($item->passed){
					...
					else if($evalType == "InstructorLegacy" && $item->confirmed){
