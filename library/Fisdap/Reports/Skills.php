	...
	private function formatPatientData($patients) {
		...
		foreach ($patients as $patient) {
            ...
            if (isset($patient['age']) || isset($patient['months'])){
                ...
                } else if ($patient['age'] == 0 && $patient['months'] >= $this->goalset->infant_start_age) {
                    $age = "<span class='hidden'>01</span>Infant:";
                } else if ($patient['age'] < $this->goalset->preschooler_start_age) {
                    $age = "<span class='hidden'>02</span>Toddler:";
                } else if ($patient['age'] < $this->goalset->school_age_start_age) {
                    $age = "<span class='hidden'>03</span>Preschooler:";
                } else if ($patient['age'] < $this->goalset->adolescent_start_age) {
                    $age = "<span class='hidden'>04</span>School age:";
                } else if ($patient['age'] < $this->goalset->adult_start_age) {
                    $age = "<span class='hidden'>05</span>Adolescent:";
                } else if ($patient['age'] < $this->goalset->geriatric_start_age) {
		...
		while($row = $result->fetch()) {
			...
			switch ($skillType) {
				...
				case 'Cardiac':
					...
					if ($procedure_name) {
						if ($row['method']) { $procedure_name .= " - ".lcfirst($row['method']); }
						if ($row['pacing_method']) { $procedure_name .= " - ".lcfirst($row['pacing_method']); }
				...
				case 'Venous':
					...
					if ($row['side']) { $site_name .= " - ".$row['side']; }
