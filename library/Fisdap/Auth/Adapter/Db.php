...
class Fisdap_Auth_Adapter_Db implements AdapterInterface
{
	...
	public function authenticate()
	{
		...
		if ($user->id) {
			if (User::authenticate_password($username, $this->password, $this->passwordHashed)) {
				...
				$result = new Zend\Authentication\Result(Zend\Authentication\Result::SUCCESS, $username, array());
			} else {
				$result = new Zend\Authentication\Result(Zend\Authentication\Result::FAILURE_CREDENTIAL_INVALID, null, array("Authentication unsuccessful. Password doesn't match."));
			}		
		} else {
			$result = new Zend\Authentication\Result(Zend\Authentication\Result::FAILURE_IDENTITY_NOT_FOUND, null, array("Authentication unsuccessful. Username not found."));			
		}
	...
	protected static function forceLogin($username = '', $password = '') {
		...
		if (!$newcreds) {
			if (count(self::$sess->identities) == 0) {
				return new Zend\Authentication\Result(Zend\Authentication\Result::FAILURE_IDENTITY_NOT_FOUND);
			}
	...
	public static function clearIdentity() {
		self::getSess();
		Zend\Authentication::getInstance()->clearIdentity();
		array_pop(self::$sess->identities);
        $username = '';
		if (count(self::$sess->identities) > 0) {
