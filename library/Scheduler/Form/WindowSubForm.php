...
class Scheduler_Form_WindowSubForm extends Fisdap_Form_Base
{
	...
	public function init()
	{
		...
		if($event){
			...
			foreach($profession_certs as $cert){
				...
				if($is_admin){$add_cert = true;}
				else {$add_cert = ($event->cert_levels & $cert->bit_value) ? true : false;}
		...
		if($defaults['cert_constraints']){
			...
			foreach($cert_constraint_value_options as $val => $label){$default_cert_values[] = $val;}
		}
		...
		for($i = 1; $i < 3; $i++){
			...
			if($i != 1){$group_constraint_values_select->setValue($default_group_values);}
		...
		foreach($this->offsets as $time){
			...
			else if($defaults['offset_type_' . $time] == 2){
				$this->{'show_' . $time . '_static'} = "style='display:none;'";
				$this->{'show_' . $time . '_interval'} = "";
				$this->{'show_' . $time . '_prevMonth'} = "style='display:none;'";
			}
			else if($defaults['offset_type_' . $time] == 3) {
				$this->{'show_' . $time . '_static'} = "style='display:none;'";
				$this->{'show_' . $time . '_interval'} = "style='display:none;'";
				$this->{'show_' . $time . '_prevMonth'} = "";
			}
            else if($defaults['offset_type_' . $time] == 4) {
                $this->{'show_' . $time . '_static'} = "style='display:none;'";
                $this->{'show_' . $time . '_interval'} = "style='display:none;'";
                $this->{'show_' . $time . '_prevMonth'} = "style='display:none;'";
	...
	private function getPrevMonthOptions()
	{
		...
		for($i = 1; $i < 32; $i++){
			...
			if($i == 1 || $i == 21 || $i == 31){$prev_month_options[$i] .= "st";}
			else if($i == 2 || $i == 22){$prev_month_options[$i] .= "nd";}
			else if($i == 3 || $i == 23){$prev_month_options[$i] .= "rd";}
			else {$prev_month_options[$i] .= "th";}
		}
