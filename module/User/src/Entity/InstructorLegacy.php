...
class InstructorLegacy extends RoleData
{
     ...
     * @return boolean
     */
    public function hasPermission($permission, $entitymanager=null)
    {
		//Load the permission cache if it hasn't been initialized
		if (empty(self::$permissionCache)) {			
			self::loadPermissionCache($entitymanager);
		}
        ...
        if (is_int($permission)) {
			...
			$permission = $entitymanager->getRepository(Permission::class)->findOneById($permission);
        } else {	
            //$permission = EntityUtils::getRepository('Permission')->findOneByName($permission);
			$permission = $entitymanager->getRepository(Permission::class)->findOneByName($permission);
	 ...
	 * @return void
	 */
	public static function loadPermissionCache($entitymanager=null)
	{
