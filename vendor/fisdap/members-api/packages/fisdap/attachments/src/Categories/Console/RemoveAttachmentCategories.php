...
/**
 * Console command for removing attachment categories
 *
 * @package Fisdap\Attachments\Categories\Console
 * @author  Ben Getsug <bgetsug@fisdap.net>
 */
final class RemoveAttachmentCategories extends Command
{
    /**
     * @inheritdoc
     */
    protected $name = 'attachments:categories:remove';

    /**
     * @inheritdoc
     */
    protected $description = 'Remove one or more attachment categories.';
    ...
    public function fire()
    {
        ...
        $this->dispatcher->dispatch(new DeleteAttachmentCategoriesCommand($attachmentType, null, $names));

        $categoryInflection = (count($names) > 1) ? Inflector::pluralize('category') : 'category';

        $this->info("Deleted '$attachmentType' attachment $categoryInflection named " . implode(', ', $names));
    }
