 ...
 * @author bgetsug
 */
class AliceDoctrineSeederLaravel5 extends Module
{
    /**
     * @var array
     */
    ...
    public function _beforeSuite()
    {
        if (! $this->hasModule('Laravel5')) {
            throw new \Exception('Laravel5 module unavailable');
        }

        /** @var Laravel5 $laravel */
        $laravel = $this->getModule('Laravel5');

        $laravel->app->boot();

        if (! $laravel->app->bound('Doctrine\ORM\EntityManager')) {
            throw new \Exception("'doctrine' not found in Laravel IoC/Service Container");
        }

        self::$em = $laravel->grabService('Doctrine\ORM\EntityManager');
    ...
    public function _before()
    {
        ...
        $laravel->app->instance('Doctrine\ORM\EntityManager', self::$em);
