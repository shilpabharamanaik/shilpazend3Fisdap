...
TableTools.prototype = {
	...
	"_fnGetDataTablesData": function ( oConfig )
	{
		...
		if ( oConfig.bHeader )
		{
			// FISDAP HACK: getting all table header rows in export data, instead of just one header row
                        // per http://stackoverflow.com/questions/7660683/tabletools-only-exporting-single-header-row/14813085#14813085
                        // @todo still need to handle colspan properly
    for (i = 0, rowCount = dt.nTHead.rows.length; i < rowCount; i++) {
        aRow = []; //clear row data
        for (j = 0, iLen = dt.aoColumns.length; j < (iLen-1); j++) {
            if (aColumnsInc[j] && typeof(dt.nTHead.rows[i].children[j]) != 'undefined' && dt.nTHead.rows[i].children[j] !== null) {
